...
public class BOJ_15644 {
	...
	static class Location{
		...
		public Location(int redR, int redC, int blueR, int blueC, int cnt, String command) {
			this.redR = redR;
			this.redC = redC;
			this.blueR = blueR;
			this.blueC = blueC;
		...
		while(!queue.isEmpty()) {
			...
			for(int i=0; i<4; i++) {
				...
				if(!isVisited[redNr][redNc][blueNr][blueNc]) {
					...
					queue.offer(new Location(redNr, redNc, blueNr, blueNc, now.cnt+1, now.command+temp));
				}
